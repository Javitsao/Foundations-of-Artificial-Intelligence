...
class CallPlayer(BasePokerPlayer):
    def __init__(self, **config):
        ...
        with open(os.path.join(os.path.dirname(__file__), "robbi.json"), "r") as f:
    ...
    def bet(self, amount, valid_actions, round_state):
        ...
        return "raise", max(min(amount * mul, valid_actions[2]["amount"]["max"]), valid_actions[2]["amount"]["min"])
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        card2 = max(ranks)
        print("333")
        status = self.find_status(card1, card2, same_suit, self.card_combinations)
        ...
        print(f"Card1: {card1}, Card2: {card2}, Same Suit: {same_suit}, Status: {status}, Status_sb_bet: {status_sb_bet}, Status_sb_4bet: {status_sb_4bet}, Status_bb_3bet: {status_bb_3bet}")
        ...
        if round_state["community_card"] == []:
            ...
            if self.get_my_seats(round_state['seats']) == round_state["small_blind_pos"]:
                if self.bot_raise == 0:
                    if status_sb_bet == 1:
                        return self.bet(BB * 2.5, valid_actions, round_state)
                    else:
                        return "fold", 0
                elif self.bot_raise == 1:
                    if status_sb_4bet == 1:
                        if self.bot_raise_amount <= 13:
                            return self.bet(self.bot_raise_amount * BB * 2.1, valid_actions, round_state)
                        else:
                            return self.bet(valid_actions[2]["amount"]["max"], valid_actions, round_state)
                ...
                if self.bot_raise == 0:
                    #return self.bet(BB * 4, valid_actions, round_state)
                    if status_bb_3bet == 1:
                        return self.bet(BB * 4, valid_actions, round_state)
                    else:
                        return "call", valid_actions[1]["amount"]
                    ...
                    if status_bb_3bet == 1:
                        return self.bet(self.bot_raise_amount * BB * 4, valid_actions, round_state)
