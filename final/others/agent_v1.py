...
class CallPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if round_state["community_card"] == []:
            if status == 1:
                print(round_state["pot"]["main"]["amount"] * 2)
                return bet(round_state["pot"]["main"]["amount"] * 2, valid_actions)
            else:
                if round_state["pot"]["main"]["amount"] >= 4*BB and round_state["round_count"] < 18:
                    return "fold", 0
                else:
                    # if round_state["round_count"] > 12 and mystack < botstack:
                    #     return "call", valid_actions[1]["amount"]
                    # if round_state["pot"]["main"]["amount"] == 15:
                    #     return "call", valid_actions[1]["amount"]
